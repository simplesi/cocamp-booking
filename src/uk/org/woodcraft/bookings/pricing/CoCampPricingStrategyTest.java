...
public class CoCampPricingStrategyTest extends BaseFixtureTestCase {
	...
	public void testPriceOf() {
		...
		assertEquals("Regular bookings are 150", 150, pricer.priceOf(regularBooking));
		...
		assertEquals("Bookings capped at 150 including pre and post camp", 150, pricer.priceOf(bookingWithPreAndPostCamp));
		...
		assertEquals("1 night", 35 + 15, pricer.priceOf(booking));
		
		booking.setArrivalDate(DateUtils.getDate(2011, 6, 30));
		booking.setDepartureDate(DateUtils.getDate(2011, 7, 1));
		assertEquals("2 nights", 35 + 30, pricer.priceOf(booking));

		booking.setArrivalDate(DateUtils.getDate(2011, 6, 30));
		booking.setDepartureDate(DateUtils.getDate(2011, 7, 8));
		assertEquals("9 nights", 150, pricer.priceOf(booking));
		...
		assertEquals("Bookings after the deadline cost more", 175, pricer.priceOf(lateBooking));
		...
		assertEquals("Elfins are not free", 150, pricer.priceOf(woodchipBooking));
		
		woodchipBooking.setDob(DateUtils.getDate(2005, 6, 31)); // Latest possible date to be 5 at start of camp
		assertEquals("Woodchips are free", 0, pricer.priceOf(woodchipBooking));
		
		woodchipBooking.setBookingCreationDate(TestConstants.DATE_AFTER_DEADLINE);
		assertEquals("Late woodchip bookings have an admin fee", 25, pricer.priceOf(woodchipBooking));
